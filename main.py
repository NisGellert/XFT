...
from user_input import * # Choose desired user input file 
...
if save_as_txt[0]:   
    ...
    #for k in range(len(elements)):
     #   n_bf[k+1] = oc(lambda_i,rho_bf[k],number_of_atoms[k],elements[k], optical_constants_dir)    
    #reflectance_bf = fresnel(theta_m,lambda_i ,n_bf,z_bf,sigma_bf, ares = a_res)
    #save(save_as_txt[1],angleScan,sample_structure, theta_m, reflectance_bf, data,rho_bf, sigma_bf, z_bf, minFOM, a_res, energy_m, logFitting, weightsFitting, ang_off)
...
for i in range(len(model_space)):
    for j in range(i+1,len(model_space)):
        print(model_space [j] is model_space[i])
